...
export const AiTopicLessonModal: React.FC<AiTopicLessonModalProps> = ({
  ...
  // Model reading audio states for narrative story
  const [isPlayingStoryReading, setIsPlayingStoryReading] = useState<boolean>(false);
  const [storyAudioSpeed, setStoryAudioSpeed] = useState<number>(0.9);

  // Stop audio on tab change or modal close
  useEffect(() => {
    return () => {
      SpeechService.stop();
      setIsPlayingStoryReading(false);
    };
  }, []);

  const handleToggleStoryReading = () => {
    if (isPlayingStoryReading) {
      SpeechService.stop();
      setIsPlayingStoryReading(false);
      return;
    }
    if (!lesson?.connectingNarrative?.text) return;
    SpeechService.stop();
    setIsPlayingStoryReading(true);
    SpeechService.speak(lesson.connectingNarrative.text, {
      rate: storyAudioSpeed,
      onEnd: () => setIsPlayingStoryReading(false),
      onError: () => setIsPlayingStoryReading(false),
    });
  };
          ...
          {lesson && !isLoading && (
            <div className="space-y-6">
              ...
              {activeTab === "story" && lesson.connectingNarrative && (
                <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm space-y-4">
                  {/* Model Audio Reading Banner */}
                  <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-blue-950 rounded-2xl p-4 sm:p-5 text-white shadow-md border border-blue-800/40 flex flex-wrap items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      <div className="w-11 h-11 rounded-xl bg-blue-600/30 border border-blue-400/40 flex items-center justify-center text-sky-400 shrink-0">
                        <Headphones className="w-5 h-5" />
                      </div>
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="text-[11px] font-black uppercase tracking-wider text-sky-300 font-mono">
                            PHẦN ĐỌC MẪU BẢN XỨ (SAMPLE AUDIO READING)
                          </span>
                          {isPlayingStoryReading && (
                            <span className="px-2.5 py-0.5 rounded-full bg-emerald-500 text-[10px] font-mono font-bold text-white uppercase animate-pulse flex items-center gap-1.5">
                        ...
                        <p className="text-xs text-slate-300 font-medium mt-0.5">
                          Luyện nghe ngữ điệu và phát âm chuẩn IELTS qua bài đọc mẫu kết nối từ vựng theo ngữ cảnh.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2.5 flex-wrap">
                      ...
                      <div className="flex items-center bg-slate-800/90 rounded-xl p-1 border border-slate-700 text-xs">
                        <span className="text-slate-400 font-bold px-2 text-[11px]">Tốc độ:</span>
                        {[0.8, 1.0, 1.2].map((rate) => (
                          <button
                            key={rate}
                            onClick={() => {
                              setStoryAudioSpeed(rate);
                              if (isPlayingStoryReading) {
                                SpeechService.stop();
                                setIsPlayingStoryReading(false);
                              }
                      ...
                      <button
                        ...
                        className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all shadow-md flex items-center gap-2 cursor-pointer ${
                          isPlayingStoryReading
                      ...
                      >
                        <Volume2 className="w-4 h-4" />
                        <span>{isPlayingStoryReading ? "Dừng Đọc Mẫu" : "Phát Đọc Mẫu"}</span>
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="p-4 sm:p-5 bg-amber-50/40 rounded-xl border border-amber-200/60 font-serif leading-relaxed text-slate-900 text-sm sm:text-base shadow-xs">
                    {lesson.connectingNarrative.text}
                  </div>
