...
import { B2_EQUIVALENTS_MAP } from "../data/b2Equivalents";
...
// Enable CORS for Vercel, cross-origin requests and preview
...
// 2. AI-powered IELTS Reading Text Analyzer
router.post("/analyze-text", async (req, res) => {
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      console.warn("[Gemini API] GEMINI_API_KEY not configured. Serving grounded academic roots analysis.");
      const fallbackRoots = generateFallbackTextAnalysis(text);
      return res.json(fallbackRoots);
    }
...
router.post("/generate-lesson", async (req, res) => {
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      console.warn("[Gemini API] GEMINI_API_KEY not configured. Serving grounded linguistics masterclass lesson.");
      return res.json(fallbackLessonGenerator());
    }
...
8. Study Tips: 2-3 lời khuyên từ chuyên gia ngôn ngữ học để tránh lỗi sai và tối ưu hóa ghi nhớ ngắt quãng.`;
    ...
    const { parsed } = await callGeminiWithRetryAndFallback(ai, {
      ...
      fallbackGenerator: fallbackLessonGenerator
    });
...
// 4. AI-powered Personalized Study Recommendations
router.post("/api/study-recommendations", async (req, res) => {
  try {
    const { 
      currentStats, 
      targetBand = "Band 8.0", 
    ...
    const fallbackRecommendationsGenerator = () => ({
    ...
    if (!process.env.GEMINI_API_KEY) {
      console.warn("[Gemini API] GEMINI_API_KEY not configured. Serving personalized study recommendations blueprint.");
      return res.json(fallbackRecommendationsGenerator());
    }
...
5. ieltsExamStrategies: 3 chiến lược thực chiến cho Writing Task 2, Reading và Speaking.`;
    ...
    const { parsed } = await callGeminiWithRetryAndFallback(ai, {
      ...
      fallbackGenerator: fallbackRecommendationsGenerator
    });
...
// 5. AI-powered Linguistic Q&A (Ask the Etymology Professor)
router.post("/ask-linguist", async (req, res) => {
  try {
    ...
    const fallbackLinguistGenerator = () => ({
    ...
    if (!process.env.GEMINI_API_KEY) {
      console.warn("[Gemini API] GEMINI_API_KEY not configured. Serving grounded linguistic Q&A advice.");
      return res.json(fallbackLinguistGenerator());
    }
...
4. ieltsApplications (tùy chọn): Mẹo áp dụng trong Writing Task 2, Reading và ví dụ collocation nâng cao chuẩn Band 8.0+.`;
    ...
    const { parsed } = await callGeminiWithRetryAndFallback(ai, {
      ...
      fallbackGenerator: fallbackLinguistGenerator
    });
...
// 6. AI-powered Topic Vocabulary Mnemonic Lesson Generator
router.post("/generate-topic-lesson", async (req, res) => {
  try {
    const { 
      topicName, 
    ...
    if (!process.env.GEMINI_API_KEY) {
      console.warn("[Gemini API] GEMINI_API_KEY not configured. Serving grounded mnemonic topic lesson.");
      const fallbackTopic = generateFallbackTopicLesson({
        topicName,
        macroDomain,
        words,
        targetBand,
...
// 7. AI Interactive IELTS Vocabulary Tutor (Gia sư AI Học Từ Vựng Chuyên Sâu)
router.post("/ai-tutor", async (req, res) => {
  try {
    ...
    const fallbackTutorGenerator = () => generateFallbackAiTutorLesson({
      userPrompt,
      currentTopic,
      selectedWords,
      macroDomain,
    ...
    if (!process.env.GEMINI_API_KEY) {
      console.warn("[Gemini API] GEMINI_API_KEY not configured. Serving grounded AI Tutor master lesson.");
      const parsedFallback = fallbackTutorGenerator();
      if (!parsedFallback.id) parsedFallback.id = `tutor_lesson_${Date.now()}`;
      if (rootContext) (parsedFallback as any).rootContext = rootContext;
...
1. Trích xuất hoặc đề xuất ĐÚNG ${requestedWordCount} TỪ VỰNG HỌC THUẬT C1/C2 (keyTargetWords) trọng tâm nhất cho chủ đề/gốc từ này. Mỗi từ có:
   ...
   - ieltsCollocation: Cụm Collocation tự nhiên điểm cao (Band 7.5+)
2. readingPassage: Đoạn văn học thuật chuẩn mực IELTS Reading dài khoảng 130 - 180 từ, chia làm 3-4 đoạn nhỏ (sections).
3. inDepthAnalysis: Phân tích sâu 3 từ vựng chủ đạo (morphology, wordFamily, academicRegister, examPitfalls, sentenceFrames).
4. diverseTestQuestions: 4-5 câu hỏi trắc nghiệm kiểm tra đa dạng (ngữ cảnh, paraphrase, collocation, tìm lỗi sai, định nghĩa).
5. digitalCrossLinks: 3 liên kết mở rộng học thuật (Roots Hub, Paraphrase, Listening traps).
6. b2ToC1Upgrades: 4 cặp biến đổi từ B2 sang C1/C2.
7. mnemonicStory: Câu chuyện ngắn ghi nhớ sâu.
8. tutorChallenge: 1 câu hỏi thử thách phản xạ nhanh kèm giải thích.
9. tutorAdvice: Lời khuyên thiết thực.
10. suggestedFollowUps: 3 câu hỏi gợi ý tiếp theo.`;

    const { parsed } = await callGeminiWithRetryAndFallback(ai, {
      prompt,
      systemInstruction: "Bạn là Gia Sư AI IELTS Quốc Tế và Chuyên Gia Ngôn Ngữ Học Cấp Cao. Bạn tạo các bài học từ vựng chuẩn mực, truyền cảm hứng, sắc sảo về mặt từ nguyên học và có tính ứng dụng thực chiến tối đa cho kỳ thi IELTS.",
      responseMimeType: "application/json",
      responseSchema: {
        type: Type.OBJECT,
        properties: {
          ...
          keyTargetWords: {
            ...
            items: {
              ...
              required: ["word", "vietnamese", "b2Equivalent", "memoryHook", "ieltsCollocation"]
            }
          },
          readingPassage: {
            type: Type.OBJECT,
              ...
              sections: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    takeawayTip: { type: Type.STRING }
                  },
                  required: ["sectionNumber", "sectionTitle", "textEn", "textVi", "keyWordsInSection"]
                }
              }
            },
            required: ["title", "fullTextEn", "fullTextVi", "sections"]
          },
          inDepthAnalysis: {
            type: Type.ARRAY,
            items: {
          ...
          digitalCrossLinks: {
            ...
            items: {
              type: Type.OBJECT,
              properties: {
                type: { type: Type.STRING, enum: ["root", "paraphrase", "listening"] },
                title: { type: Type.STRING },
                detail: { type: Type.STRING },
                badge: { type: Type.STRING }
              },
              required: ["type", "title", "detail", "badge"]
            }
          },
          b2ToC1Upgrades: {
            type: Type.ARRAY,
      ...
      fallbackGenerator: fallbackTutorGenerator
    });
...
// Mount router on BOTH "/api" AND "/" to handle any proxy / rewrite behavior seamlessly
app.use("/api", router);
app.use("/", router);
